...
export function SmoothCursor({
    ...
    useEffect(() => {
        const interactiveSelector = [
            ...
            "select", "textarea", "label", "[contenteditable]"
        ].join(", ");
        ...
        const isInteractiveElement = (node) => {
            ...
            while (el && el !== document.body) {
                ...
                if (el.hasAttribute("data-cursor") && el.getAttribute("data-cursor") === "interactive") return true;
        ...
        const onPointerOver = (e) => {
            ...
            // 1) On remonte d'abord vers un éventuel hôte d'icône custom
            const iconHost = el.closest ? el.closest('[data-cursor-icon]') : null;

            // 2) La zone est interactive si la cible directe est interactive
            //    OU si on a trouvé un hôte avec data-cursor-icon (ex: le <a>)
            ...
            if (interactive) {
                ...
                setCursorIcon(custom && custom.length ? custom : 'arrow');
        ...
        const onPointerOut = (e) => {
            ...
            if (!next || !isInteractiveElement(next)) {
                ...
                setCursorIcon("arrow"); // reset to default for next hover
            }
        };
    ...
    useEffect(() => {
        diameter.set(isInteractive ? 44 : 22); // morph fluide base ↔ interactif
    ...
    return (
        <motion.div
            style={{
                ...
                zIndex: 100,
                pointerEvents: "none",
                // 👇 Effet verre dépoli (important)
                background: GLASS_BG,
                backdropFilter: "blur(6px) saturate(120%)",
                WebkitBackdropFilter: "blur(6px) saturate(120%)",
                border: GLASS_BORDER,
                // léger relief interne (très soft)
                boxShadow:
